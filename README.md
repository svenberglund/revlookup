...
The first few examples in the list shows a typical situation where the `host` resolved is actually a *content delivery network host* while the `cn` field in these cases indeed manages to achieve the "cdn transparency" that we want, hence it will reveal the service provider on the endpoint.\
